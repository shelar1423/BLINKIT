...
/**
 * Imported — the campaign file's own trolley-and-star.
 *
 * Drawn as one shape rather than as strokes, so it is masked rather than
 * filled: the two dots at the wheels are HOLES in the artwork, and on a
 * coloured band a hole has to let the band through. Painting them white would
 * only work over white. The mask carries the body in white and the holes in
 * black, and a single currentColor rect is what actually gets painted — which
 * also means the icon inherits the rail's colour like every other one.
 *
 * Kept on the 48 grid it was drawn on: its artwork already measures about 17.3
 * against the 24 box once scaled, which is the size Beauty and Gifting sit at,
 * so it needs no fitting of its own.
 */
export const IconImported = ({ size = 20, className }: P) => {
  /* useId hands back ":r1:", and a colon inside url(#...) is not something
     every engine will resolve — a mask that fails to resolve here does not
     degrade, it paints the whole 48px square. Stripped to letters and digits. */
  const id = 'imp' + useId().replace(/[^a-zA-Z0-9]/g, '');
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" className={className} aria-hidden="true" focusable="false">
      <mask id={id} maskUnits="userSpaceOnUse" x="0" y="0" width="48" height="48">
        <path
          d="M4.83391 7.17534C5.50827 7.09156 6.18123 7.36894 6.8364 7.52855C7.44948 7.67791 8.2002 7.78099 8.78823 7.98275C9.07539 8.07955 9.34921 8.21212 9.60327 8.37738C10.7414 9.12287 11.0721 10.3533 11.332 11.6081C12.7584 11.5481 14.3285 11.5797 15.7608 11.6195C17.0663 11.6558 17.3405 13.4612 16.123 13.8166C15.7866 13.9148 14.7676 13.8712 14.3697 13.8689C13.5141 13.8583 12.6584 13.8597 11.8028 13.8729C12.3117 15.979 12.777 18.2807 13.2417 20.4107L14.656 26.6884C14.7586 27.1583 14.8052 27.5806 14.9085 28.0399C19.1476 28.1161 23.6014 28.0448 27.8572 28.046L32.92 28.0499C33.6999 28.0504 36.1752 28.1982 36.3393 27.7235C37.5879 24.0827 38.6106 20.2696 39.7898 16.6034C40.3525 14.854 38.6877 14.0205 37.2542 14.0605C35.842 14.0999 34.5484 13.2384 35.7205 11.9116C36.034 11.5568 37.4402 11.7807 37.8739 11.8291C38.9985 11.9165 40.2352 12.1811 41.0683 13.0418C42.1717 14.1417 42.5422 15.6677 42.1035 17.1504C41.4921 19.2598 40.8283 21.3449 40.1879 23.442L39.3121 26.2991C39.149 26.8434 38.9929 27.3854 38.8165 27.9262C37.8645 30.8445 35.1556 30.3422 32.6826 30.3413L25.754 30.338L15.4959 30.3414C15.6483 31.2951 15.8553 33.1638 16.6926 33.7451C17.1514 34.0636 19.1525 33.9811 19.7805 33.9801L23.4271 33.9777L34.5601 33.9796C35.3647 33.978 37.7812 33.9106 38.5001 34.0037C38.7138 34.0307 38.9151 34.119 39.0797 34.2579C39.7487 34.8274 39.5866 36.0664 38.7742 36.2212C38.4578 36.2816 37.2763 36.2366 36.8822 36.2411C37.2751 37.0969 37.3991 37.9427 37.097 38.8503C36.7657 39.8302 36.0635 40.6412 35.141 41.109C34.2562 41.5564 33.2302 41.6347 32.2877 41.3268C31.3312 41.0047 30.652 40.3447 30.2153 39.4503C29.6651 38.3233 29.7807 37.3635 30.1656 36.2325C27.8558 36.289 25.3793 36.2243 23.046 36.2403C23.3356 37.3892 23.5416 38.093 23.0271 39.2811C22.6438 40.1662 21.7702 40.9467 20.8666 41.2749C19.9317 41.6243 18.8957 41.5845 17.9903 41.1644C16.0973 40.2736 15.5085 37.9837 16.392 36.1576C13.5839 34.936 13.4424 32.1371 12.8446 29.4952C11.8733 25.2022 10.9121 20.9058 9.95957 16.6085L9.22888 13.2473C9.02027 12.2672 8.92378 11.1338 8.26306 10.3507C7.20426 10.0868 6.14279 9.83348 5.08933 9.54647C4.28427 9.32719 3.80994 8.71333 4.07278 7.87742C4.19029 7.5036 4.5024 7.33139 4.83391 7.17534Z"
          fill="#fff"
        />
        <path
          d="M25.3026 7.01983C26.3551 6.8586 26.4034 7.70978 26.6919 8.51494C27.0243 9.43285 27.3406 10.3565 27.6407 11.2855C28.8652 11.2881 30.7918 11.1813 31.9084 11.4336C33.688 11.8357 30.7903 13.7038 30.4455 13.9945C30.2327 14.174 28.9482 15.0695 28.9611 15.2304C29.0779 16.68 30.1876 18.2706 29.8916 19.7315C29.8748 19.8151 29.7103 19.9881 29.6056 19.9955C28.4789 19.9056 26.6209 18.1977 25.6514 17.6816L25.5577 17.6104C25.2754 17.5844 22.8644 19.3367 22.4237 19.6128C22.1148 19.8065 21.6832 20.0183 21.3082 19.9155C21.1988 19.8856 21.1171 19.8317 21.065 19.7289C20.7971 19.2009 21.9891 15.8991 22.2175 15.2138C21.4892 14.614 20.7224 14.0552 19.9861 13.4555C19.5494 13.0998 18.7512 12.5899 18.6993 11.9697C18.6678 11.5935 19.3771 11.3513 19.6766 11.3344C20.9182 11.2641 22.2494 11.29 23.4937 11.2887C24.0221 9.96999 24.3864 8.55992 24.9975 7.27383C25.0393 7.18603 25.2182 7.07286 25.3026 7.01983Z"
          fill="#fff"
        />
        <path
          d="M19.3421 36.2841C20.2132 36.109 21.0601 36.6772 21.2284 37.5497C21.3966 38.4222 20.8217 39.2646 19.9479 39.4259C19.2601 39.5528 18.5697 39.2183 18.243 38.6C18.0074 38.1543 17.9958 37.6236 18.2115 37.168C18.4277 36.7114 18.8468 36.3837 19.3421 36.2841Z"
          fill="#000"
        />
        <path
          d="M33.2279 36.2897C34.092 36.1157 34.9335 36.6752 35.1071 37.5393C35.2808 38.4035 34.7208 39.2447 33.8566 39.418C32.9929 39.5911 32.1524 39.0316 31.9788 38.168C31.8053 37.3044 32.3645 36.4637 33.2279 36.2897Z"
          fill="#000"
        />
      </mask>
      <rect width="48" height="48" fill="currentColor" mask={`url(#${id})`} />
    </svg>
  );
};
